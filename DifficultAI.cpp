...
std::string DifficultAI::getNextStep(Board board) {
    ...
    int junqiStep[4] = { -1, 0, -1, -1 }; // [x, y, next_x, next_y]
    for (int i = 0; i < 12; i++) {
        for (int j = 0; j < 5; j++) {
    ...
    if (abs(0 - enemyX1) + abs(3 - enemyY1) < 6)//5步以内敌方最大棋子
    {
        ...
        if (finalStep[3] == -1)//翻棋逻辑
        {
            for (int i = enemyX1 + 2; i >= enemyX1 - 2; i--){
                for (int j = enemyY1 - 2; j <= enemyY1 + 2; j++){
                    if (i >= 0 && i <= 5 && j >= 0 && j <= 4){
                        if (abs(i - enemyX1)>1 || abs(j - enemyY1)>1){
                            if (nowBoard[i][j] == 0){
                                finalStep[0] = i;
                                finalStep[1] = j;
                            }
            ...
            if (finalStep[0] == -1){//没有两格子能翻的
                if (enemy1StepNum <junqiStepNum){//杀军旗步数多还是杀该棋子步数多
                    for (int i = 0; i<4; i++){
                        finalStep[i] = enemy1Step[i];
                    }
                }
                    ...
                    if (finalStep[2] == -1){//没有能杀军旗的就翻
                        int latestx, latesty, distance = 100;
                        for (int i = 0; i<6; i++){
                            for (int j = 0; j<5; j++){
                                if (nowBoard[i][j] == 0){
    ...
    if (abs(0 - enemyX2) + abs(3 - enemyY2) > 5)//5步以内不存在
    {
        std::cout<<zhaDanX<<"zhadanx";
        if (zhaDanX !=  -1)//如果有炸弹
        {
         //   std::cout<<"aaaaaaaaaaaaaaaaaaaa";
            ...
            a[0] = isValidMove(2, 1, nowBoard, 10, zhaDanX, zhaDanY);
            a[1] = isValidMove(2, 3, nowBoard, 10, zhaDanX, zhaDanY);
            a[2] = isValidMove(3, 2, nowBoard, 10, zhaDanX, zhaDanY);
            a[3] = isValidMove(4, 1, nowBoard, 10, zhaDanX, zhaDanY);
            a[4] = isValidMove(4, 3, nowBoard, 10, zhaDanX, zhaDanY);
            int retxingyingmove = 0, xyi = 0;
            for (xyi; xyi < 5; xyi++)
            {
                std::cout<<"aaaaaaaa"<<xyi;
        ...
        if (enemy2StepNum > 3)//4步及以上
        {
            if (highestLevel2 > 18)//旅长以上
            {
                    ...
                    for (int i = -1; i < 2; i++)
                    {
                        for (int j = -1; j < 2; j++)
                        {
                            if ((enemyX2 + i) >= 0 && (enemyX2 + i) <= 11)
                            {
                                ...
                                {
                                    if (nowBoard[enemyX2 + i][enemyY2 + j] > ml && nowBoard[enemyX2 + i][enemyY2 + j] > 0 && nowBoard[enemyX2 + i][enemyY2 + j] < 13)
                                    {
                ...
                if (finalStep[3] == -1)//既不杀军旗，杀军长及以上也无路径，也不能让棋子进行营规避
                {
    ...
    if (peoplehighestLevel>AIhighestLevel)//玩家最高等级高于AI
    {
        ...
        if (finalStep[3] == -1)//不杀军旗，不保护ai在人类棋盘最大的棋子
        {
            ...
            if (nx >= 0 && nx < 12 && ny >= 0 && ny < 5 && !visited[nx][ny] && isValidMove(nx, ny, nowBoard, nowBoard[startX][startY], x, y)) {
                ...
                if (g == 1){
                    if (willBeEatOneStep(nx, ny, nowBoard, nowBoard[startX][startY])){//第一步会不会被吃
                        if (nowBoard[startX][startY] > 5 && nowBoard[startX][startY] != 10){
                            continue;
                        }
                        else{
